...
class DPTable(val data: Data, val items: Int, val capacity: Int) {
  ...
  private def evalCell(key: Key, currentValue: Int): Value = {
    if (key._2 == 0) {
      ...
      else if (KnapsackSolver.estimate(key._2, data, key._1)._1 + currentValue < estimate) {
        return empty
      }
      ...
      val first = (KnapsackSolver.addDigit(1, a), b + data(key._2)._2)

      val (x, y) = getValue((key._1, key._2 - 1), currentValue)
      val second = (KnapsackSolver.addDigit(0, x), y)
