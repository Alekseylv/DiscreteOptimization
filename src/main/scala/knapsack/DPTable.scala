...
class DPTable(val data: Data, val items: Int, val capacity: Int) {
  ...
  val table: HashMap[Key, Value] = new util.HashMap(items * 2)

  def getValue(key: Key): Value = getValue(key, 0, "")

  private def getValue(key: Key, currentValue: Int, depth: String): Value = {
    if (!table.containsKey(key)) evalCell(key, currentValue, depth)
    else table.get(key)
  }

  // the meat of solver
  private def evalCell(key: Key, currentValue: Int, depth: String): Value = {
    //println(depth)
    if (key._2 == 0) {
      ...
      empty
        ...
        val (a, b) = getValue((weight, key._2 - 1), currentValue + data(key._2)._2, depth + "=")
        val first = (Solver.addDigit(1, a), b + data(key._2)._2)

        val (x, y) = getValue((key._1, key._2 - 1), currentValue, depth + "=")
